        ...
        <Document>
            ...
            <Page size="A4" style={styles.page}>
                ...
                <View>
                    ...
                    <Text style={styles.infoText}>
                        Sustainability Score: {projectData.sustainability_score.score} ({projectData.sustainability_score.rating})
                    </Text>
                </View>
                <Text style={styles.footer}>
                    Generated on {new Date().toLocaleDateString()} | Last Updated: {new Date(projectData.last_updated).toLocaleDateString()}
                </Text>
            </Page>
            ...
            <Page size="A4" style={styles.page}>
                ...
                <View style={styles.section}>
                    ...
                    {Object.entries(projectData.risk_analysis).map(([risk, level], index) => (
                        <Text key={index} style={styles.listItem}>
                            • {risk}: {level}
                        </Text>
                    ))}
                ...
                <View style={styles.section}>
                    ...
                    {projectData.policy_compliance.local_regulations.map((reg, index) => (
                        <Text key={index} style={styles.listItem}>
                            • <Text style={{ fontWeight: 'bold' }}>{reg.law_name}:</Text> {reg.compliance_status}
                            {reg.notes && `\n  Notes: ${reg.notes}`}
                        </Text>
                    ))}
                    ...
                    {projectData.policy_compliance.international_guidelines.map((guide, index) => (
                        <Text key={index} style={styles.listItem}>
                            • <Text style={{ fontWeight: 'bold' }}>{guide.treaty}:</Text> {guide.alignment}
                            {guide.notes && `\n  Notes: ${guide.notes}`}
                        </Text>
                    ))}
                </View>
                ...
                <View style={styles.section}>
                    ...
                    {projectData.funding_opportunities.map((fund, index) => (
                        <Text key={index} style={styles.listItem}>
                            ...
                            {'\n  Amount: '}{fund.amount}
                            {'\n  Deadline: '}{fund.application_deadline}
                        </Text>
                    ))}
                </View>
